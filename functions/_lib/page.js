...
export function head(title, cfg = {}, meta = {}) {
  ...
  if (meta.path) tags.push(`<link rel="canonical" href="${esc(origin + meta.path)}"/>`);
  tags.push(`<meta property="og:site_name" content="${esc(PUBLICATION)}"/>`);
  tags.push(`<meta property="og:locale" content="de_DE"/>`);
  ...
  if (meta.path) tags.push(`<meta property="og:url" content="${esc(origin + meta.path)}"/>`);
...
<script type="text/javascript" src="https://steady.page/widget_loader/${steadyId}"></script>
${analytics}
</head><body>
...
export function header({ tabs = false, activePath = "" } = {}, cfg = {}) {
  ...
  const login = `<div class="header-actions"><a class="steady-login-button" data-size="small" data-language="de"></a><a class="login-link login-link--fb" id="js-login" href="${loginUrl}">Login</a></div>`;
  ...
  const searchDialog = (tabs && cfg.search)
    ? `<dialog class="kit-search" id="kit-search" aria-label="Suche">
  <div class="kit-search__box">
    <input class="kit-search__in" id="kit-search-in" type="search" placeholder="Suchen …" autocomplete="off" spellcheck="false" aria-label="Suchbegriff"/>
    <button class="kit-search__x" id="kit-search-close" type="button" aria-label="Schließen">×</button>
  </div>
