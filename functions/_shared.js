...
<script>(function(){
  ...
  if(pal&&window.KIT_PALETTES){for(var qi=0;qi<window.KIT_PALETTES.length;qi++){(function(idx){var p=window.KIT_PALETTES[idx];var b=document.createElement("button");b.title=p.n+((window.KIT_RL&&window.KIT_RL(p.v["--color-bg"]||"#FFFFFF")<0.42)?" (dunkel)":"");var _br=p.v["--color-brand"],_bg=p.v["--color-bg"]||"#FFFFFF";b.style.background="linear-gradient(135deg, "+_br+" 0 50%, "+_bg+" 50% 100%)";b.addEventListener("click",function(){window.kitPalette(idx,true);markPal();syncColors();clearLook();});pal.appendChild(b);})(qi);}}
